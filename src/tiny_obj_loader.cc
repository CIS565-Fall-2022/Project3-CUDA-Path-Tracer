...
std::string SimpleLoadObj(std::vector<float> &position,
                                                ...
                                                const char *filename)
{
    ...
    return SimpleLoadObj(position, normal, posIndex, normIndex, ifs);
}
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'f' && isSpace((token[1]))) {
      ...
      while (!isNewLine(token[0])) {
        vertex_index vi =
            parseTriple(token, static_cast<int>(v.size() / 3), static_cast<int>(vn.size() / 3), static_cast<int>(vt.size() / 2));
        posIndex.push_back(vi.v_idx);
        normIndex.push_back(vi.vn_idx);
        size_t n = strspn(token, " \t\r");
        token += n;
      }
