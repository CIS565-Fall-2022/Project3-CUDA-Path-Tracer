...
namespace tinyobj {
    ...
    std::string QLoadObj(std::vector<shape_t>& shapes,
        std::vector<material_t>& materials, // [output]
        const char* filename, std::vector<std::vector<int>>* indices, std::vector<glm::vec3>* pos, const char* mtl_basepath) {
        ...
        return QLoadObj(shapes, materials, ifs, indices, pos, matFileReader);
    }

    std::string QLoadObj(std::vector<shape_t>& shapes,
        std::vector<material_t>& materials, // [output]
        std::istream& inStream, std::vector<std::vector<int>>* indices, std::vector<glm::vec3>* pos, MaterialReader& readMatFn) {
        std::stringstream err;
            ...
            if (token[0] == 'f' && isSpace((token[1]))) {
                ...
                while (!isNewLine(token[0])) {
                    vertex_index vi =
                        parseTriple(token, static_cast<int>(v.size() / 3), static_cast<int>(vn.size() / 3), static_cast<int>(vt.size() / 2));
                    face.push_back(vi);
                    vertexGroup.push_back(vi.v_idx);
                    size_t n = strspn(token, " \t\r");
                    token += n;
                }
                indices->push_back(vertexGroup);
                faceGroup.push_back(face);
